    ...
    Statement = [
      {
        Effect = "Allow"
        Principal = { AWS = [
          aws_iam_role.hub_ecs.arn,
          "arn:aws:iam::${var.hub_account_id}:root"
        ] }
        Action = "sts:AssumeRole"
      }
    ]
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:lambda:${var.region}:${var.spoke_account_id}:function:*"
      },
      {
        ...
        Resource = "arn:aws:iam::${var.spoke_account_id}:role/ecs-caller-*"
      },
      {
        Sid    = "SSMRead"
        Effect = "Allow"
        ...
        Resource = "arn:aws:ssm:${var.region}:${var.spoke_account_id}:parameter/hub-spoke/*"
      }
    ]
  })
}
